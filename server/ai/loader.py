...
PREFERRED_MODEL_HINTS = [
    "hanok_damage_model.pth",  # 기본 모델 파일 (우선순위 1)
    "hanok_damage_model.pt",
    "best_model.pth",  # 대체 경로
    "best_model.pt",
...
def _find_model_path():
    ...
    2. best_model.* 우선 검색
    3. 폴더인 경우 best 모델 자동 선택
    4. 기본값: 현재 디렉토리의 .pt 또는 .pth 파일
    """
...
def _find_best_checkpoint(model_dir):
    ...
    for ckpt_path in sorted(checkpoint_files):
        try:
            ...
            print(
                f"[AI]   📄 {filename} - Epoch: {epoch + 1 if epoch >= 0 else 'N/A'}, Best mAP: {best_map:.4f if best_map >= 0 else 'N/A'}"
            )
    ...
    if best_checkpoint_path:
        print(
            f"[AI] ✅ 최고 성능 모델 선택: {os.path.basename(best_checkpoint_path)} (Best mAP: {best_map_value:.4f})"
        )
...
def load_ai_model():
    ...
    try:
        ...
        if not model_path:
            ...
            print(
                f"[AI]    2. {os.path.dirname(__file__)}/ 디렉토리에 .pt 또는 .pth 파일 배치"
            )
            return False
        ...
        id2label_korean = {0: "갈램", 1: "균열", 2: "부후", 3: "압괴/터짐"}
        ...
        if "model_state_dict" in checkpoint:
            ...
            print(
                f"[AI] ⚠️  체크포인트에 'model_state_dict' 키가 없습니다. 직접 로드 시도..."
            )
